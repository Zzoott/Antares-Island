# Hi everybody !

Here is my personnal logbook for my adventures on **Tomodachi Life : Living the dream**

> ! Every Mii is a fictionnal character I imagined using the Mii editor, each look-a-like person IRL would be really funny but not my goal !

Here, you can find one directory per inhabitants in the "Characters" directory (every informations about it in the [READ-Characters](/Characters/READ-Characters.md) file).

Each .md out of the "[Days_Facts](/Days_Facts/)" directory is here to update each day, what I did and saw after I finish my session of the day. [Here you can fin the Day 1's file where I wrote everything I did in the demo](/Days_Facts/DAY-1.md)

Happy reading to you ! Hope you enjoy the logbook !!!

---

> (Yes my game is in french beacause i AM french, so don't hate me if I write a bad english especially for neutral pronouns conjugation because I didn't train my english on this due to schools in france which aren't very leftists in their school programs :/)

---

# Various notes about the game in general

- I absolutely LOVE the editor, it's really ergonomic and it really let everybody make whatever they wants.
- It's so bad that in french we don't have "iel" which is the non-binary pronoun but I imagine and hope the non presence isn't due to translators lazyness but nintendo or technical limitations. I hope...
- I HATE that the demo don't let Miis out of their houses  when you finish it (it's considered finish when you give a Mii clothes).
- The artistic direction is lovely and it runs well on the nintendo switch first owner of the name.
- UI/UX is genious, except for the touch screen we can't use where we would like in houses to take and move Miis in them.
